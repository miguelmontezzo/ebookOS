...
export default function AdminEbookManager() {
    ...
    const handleSaveSettings = async (e: React.FormEvent) => {
        ...
        const { error } = await supabase.from('ebooks').update({
            ...
            payment_provider: editCheckoutUrl ? 'stripe' : null,
    ...
    const handleSyncStripeProduct = async () => {
        try {
            if (!editTitle || !editPriceCents) {
                alert('Informe título e preço para gerar o checkout Stripe.');
                return;
            }
            ...
            const res = await fetch('/api/payments/stripe/sync-product', {
                method: 'POST',
                headers: {
            ...
            if (!res.ok) {
                alert(data?.error || 'Falha ao sincronizar com o Stripe.');
                return;
            }

            if (data.checkoutUrl) setEditCheckoutUrl(data.checkoutUrl);
            alert('Checkout Stripe gerado! Clique em "Salvar Configurações" para confirmar.');
        } catch (err: any) {
            alert('Erro ao sincronizar Stripe: ' + (err?.message || 'erro inesperado'));
        } finally {
                        ...
                        <div className="bg-zinc-800/40 p-4 sm:p-6 rounded-2xl border border-zinc-700/50">
                            <form onSubmit={handleSaveSettings} className="space-y-6">
                                ...
                                <div className="grid sm:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-zinc-300 mb-2">Link de Checkout Stripe</label>
                                        <input
                                            type="url"
                                ...
                                <div>
                                    <button
                                        type="button"
                                        onClick={handleSyncStripeProduct}
                                    ...
                                    >
                                        {isSyncingStripe ? <><Loader2 className="w-4 h-4 animate-spin" /> Gerando checkout...</> : 'Gerar checkout Stripe'}
                                    </button>
                                </div>
